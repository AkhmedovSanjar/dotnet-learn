  ...
  return (
    <div className="space-y-5">
      {questions.map((question) => {
        const reveal = result?.details.find((detail) => detail.questionId === question.id);
      ...
      <div className="rounded-[30px] border border-[color:var(--border-color)] bg-white/95 p-6 shadow-[var(--shadow-soft)] dark:bg-slate-950/70">
        {result ? (
          <div className="mb-5 rounded-[24px] bg-slate-50 p-4 dark:bg-slate-900">
        ...
        <button
          type="button"
          disabled={submitting || questions.some((question) => !answers[question.id])}
